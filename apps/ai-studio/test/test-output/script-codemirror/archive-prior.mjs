...
const reportPath = 'docs/evidence/workspace-advanced-drawer/verification.json';
const report = JSON.parse(await readFile(reportPath, 'utf8'));
const destination = 'apps/ai-studio/test/m14-integration/test-output/diagnostics/before-script-codemirror';
...
await writeFile(`${destination}/index.json`, JSON.stringify({ schemaVersion: 1, archivedAt: new Date().toISOString(), priorInputDigest: report.inputBinding.digest, reason: 'Retain Drawer verification before replacing the script textarea with CodeMirror.', entries }, null, 2) + '\n');
