...
const reportPath = 'docs/evidence/workspace-entity-tree/verification.json';
const report = JSON.parse(await readFile(reportPath, 'utf8'));
const destination = 'apps/ai-studio/test/m14-integration/test-output/diagnostics/before-workspace-advanced-drawer';
...
await writeFile(`${destination}/index.json`, JSON.stringify({ schemaVersion: 1, archivedAt: new Date().toISOString(), priorInputDigest: report.inputBinding.digest, reason: 'Retain object Tree verification before adopting the public Drawer component.', entries }, null, 2) + '\n');
