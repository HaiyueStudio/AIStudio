...
const reportPath = 'docs/evidence/task-graph-continuity/verification.json';
const report = JSON.parse(await readFile(reportPath, 'utf8'));
const destination = 'apps/ai-studio/test/m14-integration/test-output/diagnostics/before-execution-view-tabs';
...
await writeFile(`${destination}/index.json`, JSON.stringify({ schemaVersion: 1, archivedAt: new Date().toISOString(), priorInputDigest: report.inputBinding.digest, reason: 'Retain previous task-chain verification before capturing the mutually exclusive execution views.', entries }, null, 2) + '\n');
