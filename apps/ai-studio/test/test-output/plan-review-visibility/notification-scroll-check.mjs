...
app.whenReady().then(async () => {
  ...
  const evaluate = code => window.webContents.executeJavaScript('(async()=>{try{return await eval('+JSON.stringify(code)+')}catch(e){return {testError:e.message}}})()').then(value=>{if(value?.testError)throw Error(value.testError);return value;});
  ...
  await evaluate('document.querySelector("[data-kind=plan]").scrollIntoView({block:"center"})'); await frame(); await evaluate('window.reviewButtonPoint("批准并执行")');
