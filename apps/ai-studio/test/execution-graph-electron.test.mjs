...
window.runLiveFrontierCheck=async()=>{
  const liveOps=ops.slice(0,9);
  const renderLive=()=>show([projectExecutionGraph({sessionId,ops:liveOps})]);
  const settle=()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
  renderLive(); await settle();
  const toolNodes=[...root.querySelectorAll('.execution-node.kind-tool.status-running')];
  if(toolNodes.length!==2||toolNodes.some(node=>!node.querySelector('hy-border-beam')))throw new Error('Concurrent running tools lack border beams');
  if(root.querySelector('.execution-node.kind-turn hy-border-beam')||root.querySelector('.execution-node.kind-goal hy-border-beam'))throw new Error('Ancestor beam obscures the current tools');
  const beam=toolNodes[0].querySelector('hy-border-beam');const segment=beam.shadowRoot.querySelector('.beam');
  const before=getComputedStyle(segment).strokeDashoffset;await new Promise(resolve=>setTimeout(resolve,180));
  if(getComputedStyle(segment).strokeDashoffset===before)throw new Error('Tool beam does not animate');
  if(!root.querySelector('.execution-current-activity').textContent.includes('scene.diff'))throw new Error('Current activity does not name the tool');
  liveOps.push(makeOp(9,'approval.requested',{nodeId:'approval:live',payload:{approvalId:'approval:live',reason:'Allow the edit'}}));
  renderLive();await settle();
  if(!root.querySelector('.execution-node.kind-approval.status-waiting hy-border-beam')||root.querySelector('.execution-node.kind-tool hy-border-beam'))throw new Error('Approval does not own the waiting highlight');
  liveOps.push(makeOp(10,'approval.resolved',{nodeId:'approval:live',payload:{approvalId:'approval:live',resolution:'allow-once'}}));
