...
const report = await json(output + 'diagnostics/before-canvas-plane/local-acceptance.json');
...
const rootLog = await readFile(output + 'canvas-plane-root-check.log', 'utf8');
assert.ok(rootLog.includes(`${collected.executedFiles} files / ${passed} cases passed without skips`));
for (const [name, count] of [['canvas-plane-unit.log', 63], ['canvas-plane-compat.log', 15], ['canvas-plane-window.log', 1], ['canvas-plane-discovery.log', 6]]) {
...
report.rootCheck = { command: 'npm run check', exitCode: 0, log: output + 'canvas-plane-root-check.log' };
...
report.graphPanning = { ...report.graphPanning, historical: true, verifiedInputDigest: priorDigest };
report.canvasTextureAndPlanes = {
  focusedTestRuns: 85, focusedUniqueTests: 82, realCanvasWindowChecks: 1,
  tool: 'asset.generate-texture', recipeVersion: 1,
  savedProjectPngAssets: true, chineseTextAndAlphaPixels: true,
  exactRevisionAndProjectBinding: true, approvalBeforeDrawing: true,
  historyAndAssetAssignment: true, undoRedoAndReopen: true,
  rollbackAfterCommandFailure: true, lateCancelledOrChangedProjectRejected: true,
  explicitPlanes: ['xy', 'xz', 'yz'], legacyDefault: 'xy', horizontalPlane: 'xz',
  sharedAuthoringAndPreviewGeometry: true, batchCreationAndComponentRepair: true,
  exactToolNameSearchWinsOverDescriptionReferences: true,
  existingWindowRequiresRestart: true,
};
report.priorEvidenceArchive = output + 'diagnostics/before-canvas-plane/index.json';
...
const paths = [...report.evidence.map(item => item.path).filter(file => !file.startsWith(output + 'graph-pan-')), ...['canvas-plane-capability-capture.log', 'canvas-plane-root-check.log', 'canvas-plane-unit.log', 'canvas-plane-compat.log', 'canvas-plane-window.log', 'canvas-plane-discovery.log', 'canvas-texture/canvas-texture.png'].map(file => output + file)];
