...
const report = await json(output + 'diagnostics/before-execution-graph-fix/local-acceptance.json');
...
const rootLog = await readFile(output + 'execution-graph-root-check.log', 'utf8');
assert.ok(rootLog.includes(`${collected.executedFiles} files / ${passed} cases passed without skips`));
for (const [name, count] of [['execution-graph-unit.log', 29], ['execution-graph-window.log', 7]]) {
...
report.rootCheck = { command: 'npm run check', exitCode: 0, log: output + 'execution-graph-root-check.log' };
...
report.toolContractFix = { ...report.toolContractFix, historical: true, verifiedInputDigest: priorDigest };
report.executionGraphFix = {
  focusedTests: 36, chronologicalSessionOrder: true, followLatestByDefault: true,
  preservedManualHistorySelection: true, restoredEarlierTaskSessions: true,
  restoredAlreadyOpenBarrierGraphs: true, retainedSessionLimit: 50,
  fitFullGraph: true, consistentCanvasScale: true, preservedScroll: true,
  locateOffscreenStep: true, visibleContainmentEdges: true,
  realWindowChecks: ['desktop', 'narrow', 'keyboard', 'session switch', 'scroll', 'fit', 'locate'],
  noMutationReplay: true, existingWindowRequiresRestart: true,
};
report.priorEvidenceArchive = output + 'diagnostics/before-execution-graph-fix/index.json';
const archive = await json(report.priorEvidenceArchive);
assert.equal(archive.sourceInputDigest, priorDigest);
...
const replacements = new Map([
  [output + 'tool-contract-capability-capture.log', output + 'execution-graph-capability-capture.log'],
  [output + 'tool-contract-root-check.log', output + 'execution-graph-root-check.log'],
]);
const paths = [...new Set([...report.evidence.map(item => replacements.get(item.path) ?? item.path).filter(file => !file.startsWith(output + 'tool-contract-')), ...['execution-graph-unit.log', 'execution-graph-window.log', 'execution-graph-window/topology.png', 'execution-graph-window/topology.png.narrow.png'].map(file => output + file)])];
