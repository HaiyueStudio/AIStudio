...
const report = await json(output + 'diagnostics/before-graph-pan/local-acceptance.json');
...
const rootLog = await readFile(output + 'graph-pan-root-check.log', 'utf8');
assert.ok(rootLog.includes(`${collected.executedFiles} files / ${passed} cases passed without skips`));
for (const [name, count] of [['graph-pan-unit.log', 28], ['graph-pan-window.log', 2]]) {
...
report.rootCheck = { command: 'npm run check', exitCode: 0, log: output + 'graph-pan-root-check.log' };
...
report.batchLifecycleFix = { ...report.batchLifecycleFix, historical: true, verifiedInputDigest: priorDigest };
report.graphPanning = {
  focusedTests: 30, realWindowChecks: ['behavior structure', 'execution topology'],
  leftAndMiddleDrag: true, screenPixelMovementAtZoom: true,
  noNodeSelectionAfterDrag: true, clickWithSmallMovementSelectsNode: true,
  preservedScrollAfterSelection: true, pointerReleaseAndCancellation: true,
  viewportOwnedListenersAndCapture: true, preservedZoomKeyboardAndScrollbars: true,
  existingWindowRequiresRestart: true,
};
report.priorEvidenceArchive = output + 'diagnostics/before-graph-pan/index.json';
...
const paths = [...report.evidence.map(item => item.path).filter(file => !file.startsWith(output + 'batch-lifecycle-')), ...['graph-pan-capability-capture.log', 'graph-pan-root-check.log', 'graph-pan-unit.log', 'graph-pan-window.log'].map(file => output + file)];
