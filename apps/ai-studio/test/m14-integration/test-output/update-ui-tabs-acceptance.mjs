...
const report = await json(output + 'diagnostics/before-ui-tabs/local-acceptance.json');
...
const rootLog = await readFile(output + 'ui-tabs-root-check.log', 'utf8');
assert.ok(rootLog.includes(`${collected.executedFiles} files / ${passed} cases passed without skips`));
for (const [name, count] of [['ui-tabs-unit.log', 27], ['ui-tabs-window.log', 2], ['ui-tabs-package.log', 24], ['ui-tabs-product-window.log', 1], ['ui-tabs-layout-contract.log', 1]]) {
  const focused = await readFile(output + name, 'utf8');
  for (const [key, value] of [['tests', count], ['pass', count], ['fail', 0], ['cancelled', 0], ['skipped', 0]]) assert.ok(focused.includes('ℹ ' + key + ' ' + value), name + ':' + key);
}
report.verifiedAt = new Date().toISOString(); report.inputBinding = binding;
report.rootCheck = { command: 'npm run check', exitCode: 0, log: output + 'ui-tabs-root-check.log' };
...
report.games = await Promise.all(report.games.map(async game => {
  ...
  for (const round of actual.rounds) { assert.deepEqual(round.flows.map(flow => flow.signal), game.flows); assert.ok(round.flows.every(flow => flow.location)); assert.equal(round.cleanup.disposableCount, 0); }
...
report.canvasTextureAndPlanes = { ...report.canvasTextureAndPlanes, historical: true, verifiedInputDigest: priorDigest };
report.uiComponents = {
  package: '@haiyue/ui', version: '0.1.3', candidate: 'config/upstream/ui-candidate.json',
  focusedChecks: 55, uiPackageChecks: 24, shellChecks: 27, layoutContractChecks: 1, realWindowChecks: 3,
  workspaceUsesPublicTabs: true, mouseAndKeyboardSelection: true,
  rovingFocusAndAria: true, disabledTabsSkipped: true, singleChangeEvent: true,
  languageAndLayoutReconnection: true, retainedPanelIdentity: true,
  waitingCardRepeatedGraphInteractions: true, zeroComponentErrors: true,
  desktopAndNarrowScreenshots: true, existingWindowRequiresRestart: true,
  reproduction: output + 'diagnostics/ui-beam-before-fix.log',
};
report.priorEvidenceArchive = output + 'diagnostics/before-ui-tabs/index.json';
...
const paths = [...report.evidence.map(item => item.path).filter(file => !file.startsWith(output + 'canvas-plane-')), ...['ui-tabs-capability-capture.log', 'ui-tabs-root-check.log', 'ui-tabs-unit.log', 'ui-tabs-window.log', 'ui-tabs-package.log', 'ui-tabs-product-window.log', 'ui-tabs-layout-contract.log', 'ui-tabs-window/workspace-desktop.png', 'ui-tabs-window/workspace-resources.png', 'ui-tabs-window/workspace-375.png'].map(file => output + file), 'config/upstream/ui-candidate.json'];
