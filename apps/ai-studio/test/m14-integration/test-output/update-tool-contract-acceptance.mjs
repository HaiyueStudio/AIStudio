...
const report = await json(output + 'diagnostics/before-tool-contract-fix/local-acceptance.json');
...
const rootLog = await readFile(output + 'tool-contract-root-check.log', 'utf8');
assert.ok(rootLog.includes(`${collected.executedFiles} files / ${passed} cases passed without skips`));
const focused = await readFile(output + 'tool-contract-focused.log', 'utf8');
for (const [key, value] of [['tests', 46], ['pass', 46], ['fail', 0], ['cancelled', 0], ['skipped', 0]]) assert.ok(focused.includes(`ℹ ${key} ${value}`), key);
report.verifiedAt = new Date().toISOString();
report.inputBinding = binding;
report.rootCheck = { command: 'npm run check', exitCode: 0, log: output + 'tool-contract-root-check.log' };
...
report.toolFailureFix = { ...report.toolFailureFix, historical: true, verifiedInputDigest: priorDigest };
report.toolContractFix = {
  focusedTests: 46,
  exactOrderedToolContract: ['id', 'description', 'inputSchema'],
  sharedFingerprintWithBackendGuards: true,
  changedOrUnknownContractUsesFullContext: true,
  preservedApprovedPlanAndCurrentProject: true,
  unstartedProviderSessionNotReusable: true,
  backendGuardsRetained: true,
  testedBackends: ['harness', 'codex'],
  transportScope: 'Production backend adapters with local deterministic transports; no online Agent task',
  existingWindowRequiresRestart: true,
};
report.priorEvidenceArchive = output + 'diagnostics/before-tool-contract-fix/index.json';
...
const replacements = new Map([
  [output + 'tool-failures-capability-capture.log', output + 'tool-contract-capability-capture.log'],
  [output + 'tool-failures-root-check.log', output + 'tool-contract-root-check.log'],
]);
const paths = [...new Set([...report.evidence.map(item => replacements.get(item.path) ?? item.path).filter(file => !file.startsWith(output + 'tool-failures-')), output + 'tool-contract-focused.log'])];
