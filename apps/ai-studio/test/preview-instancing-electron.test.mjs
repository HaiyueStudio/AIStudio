...
function hostHtml(previewUrl) {
  ...
  const emittedText = `const blocks = api.scene.instances('entity:block', 8); blocks.set(0, { position: { x: 0, y: 0, z: 0 }, scale: { x: 4, y: 4, z: 4 } }); api.scene.hudText('score', 'SCORE 10', { position: 'top-right', color: '#7ffcff' });`;
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>html,body,iframe{margin:0;width:100%;height:100%;border:0;background:#000}</style></head><body><script>
    ...
    window.addEventListener('message', event => {
      ...
      else if (event.data.type === 'state' && event.data.tick >= 10) {
        const score = Array.isArray(event.data.hud) && event.data.hud.find(item => item && item.id === 'score');
        if (score && score.text === 'SCORE 10' && score.position === 'top-right') document.body.dataset.status = 'passed';
        else fail('responsive HUD text was not published by the preview runtime');
      }
