...
- 两条真实 Electron/WebGPU smoke 均通过：sandboxed desktop renderer/typed preload 约 `38.04 s`，browser-host-in-Electron 约
  `3.75 s`；两者均保留 renderer sandbox、GPU sandbox、context isolation、`nodeIntegration:false` 与真实 WebGPU。
