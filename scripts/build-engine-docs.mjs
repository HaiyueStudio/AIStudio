...
add('contract:studio-runtime', 'Studio 脚本入口 / onUpdate runtime contract', 'studio-script', 'studio-contract:onUpdate', ['Write a strict TypeScript onUpdate function body, without imports, exports or a lifecycle wrapper. Injected values: entity, component, world, time, delta, api. time/delta are milliseconds. Local variables reset each invocation; component.data is persistent state. api groups depend on enabled capabilities; request required capabilities through script.propose and use validation diagnostics.', 'Use api.debug listeners/timers/disposers for lifecycle cleanup. Engine native constructors and editor OrbitControl are not automatically available in Play. Discover and configure authoring components before consuming their runtime results. Match the current documentation surface and consult related types.'], '脚本入口、持久状态、时间单位、生命周期、capability 与编辑器/Play 边界', ['example:grid-placement', 'example:rotate-self', 'tool:script.propose']);
