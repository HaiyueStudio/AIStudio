...
add('example:data-first-verification', '数据优先验收 / 按对象检查与视觉截图分工', 'studio-script', 'studio-example:data-first-verification', [await readFile(path.join(root, 'docs/examples/data-first-verification.md'), 'utf8')], '验收 测试 引擎数据 按对象 截图 多模态 verification inspect capture', ['example:interaction-diagnostics', 'tool:play.inspect', 'tool:play.pointer-gesture', 'tool:play.capture', 'tool:task.evaluate']);
add('example:rotation-regression', '组合对象旋转 / 射线穿透、脚本拆分与固定回归', 'studio-script', 'studio-example:rotation-regression', [await readFile(path.join(root, 'docs/examples/rotation-and-regression.md'), 'utf8')], '魔方 拖拽 旋转 行 列 方向 多轮 修复 穿透 装饰面 脚本拆分 regression rotation dragAxis transforms penetrable', ['tool:play.regression','tool:play.pointer-gesture','component:haiyue.interaction.pointer','script:api.scene.transforms']);
add('example:interaction-diagnostics', '交互故障诊断 / 命中、事件、脚本读取与相机接管', 'studio-script', 'studio-example:interaction-diagnostics', [await readFile(path.join(root, 'docs/examples/interaction-diagnostics.md'), 'utf8')], '拖拽 无响应 相机 抢占 遮挡 修复 诊断 interaction diagnostics routing pointer gesture orbit repair', ['example:data-first-verification', 'example:drag-target', 'tool:play.pointer-gesture', 'tool:scene.get-many', 'tool:script.get']);
