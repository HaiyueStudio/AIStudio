  ...
  try {
    ...
    const observeEvent = (event) => {
      ...
      if (event.kind === 'usage') {
        ...
        const budgetedInputTokens = netNewInputTokens(totals);
        if (budgetedInputTokens > initialTranche.inputTokens && !budgetContinuations.some((entry) => entry.metric === 'inputTokens')) budgetContinuations.push(Object.freeze({ metric: 'inputTokens', basis: 'net-new-provider-input', initialLimit: initialTranche.inputTokens, extendedLimit: budget.limits.inputTokens, current: budgetedInputTokens, rawInputTokens: totals.inputTokens, cachedInputTokens: totals.cachedInputTokens, cacheWriteTokens: totals.cacheWriteTokens, authorization: 'user-authorized-formal-matrix', action: 'continue-current-preserved-task' }));
        ...
        if (budgetedInputTokens > budget.limits.inputTokens || totals.outputTokens > budget.limits.outputTokens) formalCapError ??= errorWithCode('budget.formal-cap', 'Agent exceeded the explicitly authorized formal-matrix continuation cap.');
      }
    };
      ...
      try { attempt = await executePreviewReplay({ fixture, preview, windowGuard, testCase, requireUsableVisual: repairAttempt === 0 }); break; }
        ...
        const repairPrompt = cause?.code === 'g12.replay-runtime-error'
          ...
          : cause?.code === 'g12.replay-visual-unusable'
            ? `Studio 的同 tick PNG 像素检查发现游戏画面不可用：${cause.message}。请检查持久化相机、几何体位置/尺寸/朝向、深度遮挡和角色颜色，再修复场景。目标设备为 393×852 竖屏；有明确玩法边界时优先调用 camera.author 的 frame-bounds 动作，让 Studio 按 bounds、plane、targetSize:{width:393,height:852} 和 padding 确定性计算正交相机，不能自行反转宽高或只用较短轴/半尺寸。需要独立辨识的活动/受控单元、轨迹或已落定单元、目标或收集物、边界或背景，应使用语义清晰的独立实体/实例源和显著不同材质。HaiYue plane 的局部平面是 XY；作为水平 XZ 地面时 rotationDegrees.x 必须为 -90。修复后重新 Play 并截图确认完整玩法区域、主要角色和 HUD 都实际可见。`
...
async function executePreviewReplay({ fixture, preview, windowGuard, testCase, requireUsableVisual = true }) {
  ...
  let replay;
  try { replay = await windowGuard.race(executeG12ReplayProgram(preview, replayProgram, { capture: true, maxTriggerWaitTicks: 3_600, resolveControl: (control) => replayAction(scene, control) })); }
  catch (cause) {
    const retained = await retainFailedReplayAttempt({ cause, preview, windowGuard, scene, started, replayProgram }).catch(() => null);
...
async function retainFailedReplayAttempt({ cause, preview, windowGuard, scene, started, replayProgram }) {
  const progress = cause?.replayProgress;
  if (!Array.isArray(progress?.observations) || progress.observations.length === 0) return null;
  const finalObservation = await windowGuard.race(preview.inspect());
  const capture = await windowGuard.race(preview.capture());
  if (capture.tick !== finalObservation.tick) return null;
  const observations = [...progress.observations, finalObservation].slice(-256);
  const replay = Object.freeze({
    schemaVersion: 1, replayProgramVersion: '1.0.0', baseTick: replayProgram.baseTick, finalTick: finalObservation.tick,
    semanticDriverIds: [...new Set((progress.trace ?? []).filter((entry) => entry.kind === 'semantic-driver').map((entry) => entry.driverId))].sort(),
    observedSignals: Object.freeze(Array.isArray(progress.observedSignals) ? [...progress.observedSignals] : []), observations: Object.freeze(observations), trace: Object.freeze(Array.isArray(progress.trace) ? [...progress.trace] : []), finalObservation, capture,
  });
...
function reconcileLiveUsage(store, event) { const previous = store.get(event.turnId) ?? { inputTokens: 0, cachedInputTokens: 0, cacheWriteTokens: 0, cacheReadKnown: true, outputTokens: 0 }; const mode = event.payload.mode === 'cumulative' ? 'cumulative' : 'delta'; const inputTokens = nonNegativeInteger(event.payload.inputTokens); const cachedInputTokens = nonNegativeIntegerOrNull(event.payload.cachedInputTokens); const cacheWriteTokens = nonNegativeIntegerOrNull(event.payload.cacheWriteTokens); const cacheReadKnown = previous.cacheReadKnown && cachedInputTokens !== null; store.set(event.turnId, { inputTokens: mode === 'cumulative' ? inputTokens : previous.inputTokens + inputTokens, cachedInputTokens: mode === 'cumulative' ? (cachedInputTokens ?? 0) : previous.cachedInputTokens + (cachedInputTokens ?? 0), cacheWriteTokens: mode === 'cumulative' ? (cacheWriteTokens ?? 0) : previous.cacheWriteTokens + (cacheWriteTokens ?? 0), cacheReadKnown, outputTokens: mode === 'cumulative' ? nonNegativeInteger(event.payload.outputTokens) : previous.outputTokens + nonNegativeInteger(event.payload.outputTokens) }); }
function aggregateLiveUsage(store) { return [...store.values()].reduce((sum, entry) => ({ inputTokens: sum.inputTokens + entry.inputTokens, cachedInputTokens: sum.cachedInputTokens + entry.cachedInputTokens, cacheWriteTokens: sum.cacheWriteTokens + entry.cacheWriteTokens, cacheReadKnown: sum.cacheReadKnown && entry.cacheReadKnown, outputTokens: sum.outputTokens + entry.outputTokens }), { inputTokens: 0, cachedInputTokens: 0, cacheWriteTokens: 0, cacheReadKnown: true, outputTokens: 0 }); }
function netNewInputTokens(usage) { return usage.cacheReadKnown ? Math.max(0, usage.inputTokens - usage.cachedInputTokens - usage.cacheWriteTokens) : usage.inputTokens; }
function nonNegativeInteger(value) { return Number.isSafeInteger(value) && value >= 0 ? value : 0; }
function nonNegativeIntegerOrNull(value) { return Number.isSafeInteger(value) && value >= 0 ? value : null; }
