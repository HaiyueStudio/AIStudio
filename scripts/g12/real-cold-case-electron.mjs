      ...
      try { attempt = await executePreviewReplay({ fixture, preview, windowGuard, testCase, requireUsableVisual: repairAttempt === 0 }); break; }
        ...
        const repairPrompt = cause?.code === 'g12.replay-runtime-error'
          ...
          : cause?.code === 'g12.replay-visual-unusable'
            ? `Studio 的同 tick PNG 像素检查发现游戏画面不可用：${cause.message}。请检查持久化相机、几何体位置/尺寸/朝向、深度遮挡和角色颜色，再修复场景。目标设备为 393×852 竖屏（viewportAspect=393/852）；有明确玩法边界时优先调用 camera.author 的 frame-bounds 动作，让 Studio 按 bounds、plane、viewportAspect 和 padding 确定性计算正交相机，不能只用较短轴或半尺寸。需要独立辨识的活动/受控单元、轨迹或已落定单元、目标或收集物、边界或背景，应使用语义清晰的独立实体/实例源和显著不同材质。HaiYue plane 的局部平面是 XY；作为水平 XZ 地面时 rotationDegrees.x 必须为 -90。修复后重新 Play 并截图确认完整玩法区域、主要角色和 HUD 都实际可见。`
