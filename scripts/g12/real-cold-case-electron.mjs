...
function reconcileLiveUsage(store, event) { const previous = store.get(event.turnId) ?? { inputTokens: 0, cachedInputTokens: 0, cacheWriteTokens: 0, cacheKnown: true, outputTokens: 0 }; const mode = event.payload.mode === 'cumulative' ? 'cumulative' : 'delta'; const inputTokens = nonNegativeInteger(event.payload.inputTokens); const cachedInputTokens = nonNegativeIntegerOrNull(event.payload.cachedInputTokens); const cacheWriteTokens = nonNegativeIntegerOrNull(event.payload.cacheWriteTokens); const cacheKnown = previous.cacheKnown && cachedInputTokens !== null && cacheWriteTokens !== null; store.set(event.turnId, { inputTokens: mode === 'cumulative' ? inputTokens : previous.inputTokens + inputTokens, cachedInputTokens: mode === 'cumulative' ? (cachedInputTokens ?? 0) : previous.cachedInputTokens + (cachedInputTokens ?? 0), cacheWriteTokens: mode === 'cumulative' ? (cacheWriteTokens ?? 0) : previous.cacheWriteTokens + (cacheWriteTokens ?? 0), cacheKnown, outputTokens: mode === 'cumulative' ? nonNegativeInteger(event.payload.outputTokens) : previous.outputTokens + nonNegativeInteger(event.payload.outputTokens) }); }
function aggregateLiveUsage(store) { return [...store.values()].reduce((sum, entry) => ({ inputTokens: sum.inputTokens + entry.inputTokens, cachedInputTokens: sum.cachedInputTokens + entry.cachedInputTokens, cacheWriteTokens: sum.cacheWriteTokens + entry.cacheWriteTokens, cacheKnown: sum.cacheKnown && entry.cacheKnown, outputTokens: sum.outputTokens + entry.outputTokens }), { inputTokens: 0, cachedInputTokens: 0, cacheWriteTokens: 0, cacheKnown: true, outputTokens: 0 }); }
function netNewInputTokens(usage) { return usage.cacheKnown ? Math.max(0, usage.inputTokens - usage.cachedInputTokens - usage.cacheWriteTokens) : usage.inputTokens; }
