...
test('real case runner separates model-visible request from hidden replay and persists all evidence classes', async () => {
  ...
  assert.match(source, /version: '3\.4\.0'/u);
  ...
  assert.match(source, /不能自行反转宽高或只用较短轴\/半尺寸/u);
  ...
  assert.match(source, /retainFailedReplayAttempt/u);
  assert.match(source, /cause\.replayAttempt = retained/u);
  assert.match(source, /progress\.observations.*preview\.capture/su);
  assert.match(source, /progress\.trace.*kind === 'semantic-driver'/su);
  assert.doesNotMatch(source, /semanticDriverIds:.*replayProgram\.commands/u);
  ...
  assert.match(source, /providerUnavailable \|\| hasAuthoringPlayProof\(completed\)/u);
  assert.match(source, /toolIds\.has\('play\.inspect'\) \|\| toolIds\.has\('play\.capture'\)/u);
