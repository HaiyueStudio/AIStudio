...
export const GAME_AUTHORING_TOOL_DEFINITIONS: readonly GameToolDefinition[] = Object.freeze([
  ...
  definition('entity.create', 'Create entity', 'Prepare and create one Empty or Cube through Document History.', 'reversible-edit', 'medium', schema({ baseRevision: revision, kind: { enum: ['empty', 'cube'] }, name: { type: 'string', minLength: 1, maxLength: 80 }, parentId: { anyOf: [entityId, { type: 'null' }] } }, ['baseRevision', 'kind'], ['name', 'parentId']), ['studio.scene-authoring'], 10_000, 32 * 1024, true),
  ...
  definition('script.propose', 'Propose script', 'Validate project script text and return a bounded diff proposal without committing.', 'trusted-code', 'high', schema({ baseRevision: revision, entityId, text: { type: 'string', minLength: 1, maxLength: 65536 }, capabilities: { type: 'array', maxItems: 4, uniqueItems: true, items: { enum: ['read', 'input', 'debug', 'scene-mutation'] } } }, ['baseRevision', 'entityId', 'text'], ['capabilities']), ['studio.script-preview'], 20_000, 64 * 1024, false, ['text']),
