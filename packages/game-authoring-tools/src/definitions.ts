...
export const GAME_AUTHORING_TOOL_DEFINITIONS: readonly GameToolDefinition[] = Object.freeze([
  ...
  definition('preview.validate', 'Validate preview', 'Validate a committed script and prepare a trusted preview plan without starting it. Reuse the capabilities returned by script.propose; Studio infers scene and physics from the committed source.', 'runtime-start', 'high', schema({ scriptId, capabilities: { type: 'array', maxItems: 6, uniqueItems: true, items: { enum: ['read', 'input', 'debug', 'scene', 'physics', 'asset'] } } }, ['scriptId'], ['capabilities']), ['studio.script-preview'], 20_000, 64 * 1024, false),
