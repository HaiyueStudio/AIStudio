...
export const GAME_AUTHORING_TOOL_DEFINITIONS: readonly GameToolDefinition[] = Object.freeze([
  ...
  definition('script.propose', 'Propose script', 'Validate an onUpdate function-body script and return a bounded diff proposal without committing. The text runs once per frame with entity, component, world, time, delta and api already in scope; time and delta are milliseconds. Use 1000 ms for one second and clamp long-frame delta in milliseconds. Do not use import, export, CommonJS, or a lifecycle-function wrapper. api.scene.instances(entity, capacity) controls dynamic instance data on one geometry entity. Source that uses api.scene requires the scene capability; Studio infers it, and the returned capabilities must be reused for preview.validate.', 'trusted-code', 'high', schema({ baseRevision: revision, entityId, text: { type: 'string', minLength: 1, maxLength: 65536 }, capabilities: { type: 'array', maxItems: 4, uniqueItems: true, items: { enum: ['read', 'input', 'debug', 'scene'] } } }, ['entityId', 'text'], ['baseRevision', 'capabilities']), ['studio.script-preview'], 20_000, 64 * 1024, false, ['text']),
  ...
  definition('preview.validate', 'Validate preview', 'Validate a committed script and prepare a trusted preview plan without starting it. Reuse the capabilities returned by script.propose; Studio also infers scene when the committed source uses api.scene.', 'runtime-start', 'high', schema({ scriptId, capabilities: { type: 'array', maxItems: 4, uniqueItems: true, items: { enum: ['read', 'input', 'debug', 'scene'] } } }, ['scriptId'], ['capabilities']), ['studio.script-preview'], 20_000, 64 * 1024, false),
