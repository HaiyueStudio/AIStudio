...
export function normalizeTaskAccounting(value: unknown): import('./types.js').ConversationTaskAccountingReadModel | null {
  ...
  try {
    ...
    return Object.freeze({ taskId: stable(value.taskId, 'task id'), budget, budgetStatus: status,
      usage: Object.freeze({ inputTokens: countOrNull(value.usage.inputTokens), cachedInputTokens: countOrNull(value.usage.cachedInputTokens), outputTokens: countOrNull(value.usage.outputTokens), reasoningTokens: countOrNull(value.usage.reasoningTokens), toolInputBytes: safeInteger(value.usage.toolInputBytes, 0, 1_000_000_000) ?? 0, toolOutputBytes: safeInteger(value.usage.toolOutputBytes, 0, 1_000_000_000) ?? 0, wallTimeMs: safeInteger(value.usage.wallTimeMs, 0, 86_400_000) ?? 0, ...(isRecord(value.usage.contextCache) ? { contextCache: Object.freeze({ localArtifactHits: safeInteger(value.usage.contextCache.localArtifactHits, 0, 1_000_000_000) ?? 0, localArtifactMisses: safeInteger(value.usage.contextCache.localArtifactMisses, 0, 1_000_000_000) ?? 0, deltaReuseBytes: safeInteger(value.usage.contextCache.deltaReuseBytes, 0, 1_000_000_000) ?? 0, providerCacheEligibleBytes: safeInteger(value.usage.contextCache.providerCacheEligibleBytes, 0, 1_000_000_000) ?? 0, providerReportedHitTokens: countOrNull(value.usage.contextCache.providerReportedHitTokens) }) } : {}) }),
