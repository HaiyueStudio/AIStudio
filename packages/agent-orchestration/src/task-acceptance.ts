...
export function repairRequest(task: TaskSpecV2, evaluation: EvaluationResultV2, iteration: number, interactionDiagnostic?: string): string {
  const failed = evaluation.acceptanceResults.filter((item) => item.status !== 'pass');
  return ['Continue the same visible task with a bounded evidence-led repair.', `Repair iteration: ${iteration}.`, ...(interactionDiagnostic ? [`Latest observed interaction diagnostic (historical; verify revision before reuse): ${interactionDiagnostic.slice(0, 1024)}`] : []), `Failed acceptance: ${JSON.stringify(failed.map((item) => ({ acceptanceId: item.acceptanceId, evidenceIds: item.evidenceIds, diagnostic: item.diagnostic })))}`, 'Acquire evidence per failed criterion: use play.inspect for engine data and play.pointer-gesture for interaction effects; use play.capture only for rendered appearance or a data/render discrepancy. Do not screenshot every test. Check whether a failed condition belongs to an earlier test stage: use task.evaluate acceptanceEvidence to select that stage’s retained observation (same Play and revision), rather than modifying correct gameplay or requiring playing and won at the same tick. Re-inspect the authoritative revision, change only causes supported by the cited evidence, run Play again as needed, collect fresh same-revision evidence (play.capture returns a matching screenshot/state bundle; use its observations together), and call task.evaluate with the approved criteria. For an interaction failure, read play.pointer-gesture diagnostics and steps.routing before editing. Follow geometric hit -> pointer events -> script reader/owner -> Orbit ownership -> actual entity effects. Supply expect with intended changed and unchanged members and cameraChanged; counters such as moveCount do not prove member rotation. If only a parent-local group transform changes, inspect its hierarchy and world effect rather than demanding every child local transform change. Stop repeating same-revision failures; inspect the cited target/script and change the supported cause or explain a distinct probe hypothesis. Do not repeat an unchanged repair.'].join('\n');
}
