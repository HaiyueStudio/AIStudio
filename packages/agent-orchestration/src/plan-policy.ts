...
export function approvedPlanRequest(plan: ApprovedPlanExecution, projectOpen: boolean): string {
  return [
    ...
    canonicalPlan(plan),
    'Use studio.plan.update with the exact approved item IDs to report concrete step progress before starting work and when finishing or blocking a step. Several independent steps may be in_progress. Progress is not acceptance evidence; do not mark the task accepted through this tool.',
