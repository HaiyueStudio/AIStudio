...
- Use only `@haiyue/ai-studio-shell/conversation`, `@haiyue/ai-studio-shell/advanced/model`, and `@haiyue/ai-studio-shell/resources/model` for validated headless projections; do not import renderer panels or the shell root. The boundary gate also checks these model modules cannot import presentation code.
