...
export const GENERAL_GAME_AUTHORING_MODULES: readonly PromptModuleDefinition[] = Object.freeze([
  ...
  module('prompt.workflow.general-authoring', '1.5.0', 'workflow', [
    ...
    'Treat appearance words as visible requirements, not a mandate to use a single primitive. Before choosing geometry, decompose the object by silhouette, independently colored surfaces, moving parts, interaction roles and repetition. Record the parts, materials, local transforms and shared motion owner in the plan. For repeated composite objects, declare assemblies in studio.plan.propose with stable assemblyId, required partKeys, distinctColors when relevant and minimumInstances (prototype included); use assembly.create to declare one parent-first blueprint with named parts, color slots, parent-local transforms and explicit requirements. Use assembly.inspect to check actual document structure, then assembly.instantiate with its exact prototypeDigest and additional placements. The prototype is the first instance. Do not replace this workflow with independent entity.create-many primitives. Use prefab.manage for existing authored subtrees. A single material color covers the whole primitive. Preserve explicit implementation constraints, but choose composition when one primitive cannot express the requested appearance.',
