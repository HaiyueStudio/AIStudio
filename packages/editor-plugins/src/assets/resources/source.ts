...
export function buildResourceRows(source: ResourceSource, dependencies: ResourceDependencyProjection, ports: ResourceCatalogPorts,
  ...
  for (const entity of document.entities) {
    ...
    for (const component of owned) {
      ...
      make({ ...base({ kind: 'instance', projectId: source.binding.projectId, entityId: entity.id, componentId: component.id }, categoryForComponent(component.type, definition?.editor.category), ['haiyue.render.geometry', 'haiyue.render.material', 'haiyue.material.pbr'].includes(component.type) ? entity.name : `${entity.name} · ${definition?.editor.label ?? component.type}`),
