...
export const BUILTIN_COMPONENT_DEFINITIONS: readonly ComponentDefinitionV2[] = Object.freeze([
  ...
  definition('haiyue.interaction.pointer', 'interaction.pointer', 'runtime-owner', 'medium', 'Pointer Interaction', 'Input', 'inspector.pointer-interaction', 'adapter.interaction.pointer',
    objectSchema({ events: { type: 'array', minItems: 0, maxItems: 8, description: 'Use [] with penetrable=true for decorative meshes: no script or event handler is required.', items: { enum: ['hover', 'click', 'move', 'down', 'up', 'drag', 'wheel', 'cancel'] } }, draggable: { type: 'boolean' }, capturePointer: { type: 'boolean' }, penetrable: { type: 'boolean', description: 'Skip this mesh during pointer raycast, allowing hits on the mesh behind it. Keep this component enabled. Does not hide the mesh or change physics.' }, maxEventsPerTick: { type: 'integer', minimum: 1, maximum: 256 } }, ['events', 'draggable', 'capturePointer', 'penetrable', 'maxEventsPerTick']),
