...
export function checkedJson(input: unknown, maxBytes = 8 * 1024 * 1024): unknown {
  ...
  function walk(value: unknown, depth: number): unknown {
    ...
    if (typeof value === 'number') {
      ...
      addBytes(Buffer.byteLength(JSON.stringify(value))); return value;
    }
    if (typeof value === 'string') {
      addBytes(Buffer.byteLength(JSON.stringify(value)));
    ...
    for (const key of Object.keys(descriptors)) {
      ...
      if (!Array.isArray(value)) addBytes(Buffer.byteLength(JSON.stringify(key)) + 1);
  ...
  if (Buffer.byteLength(canonicalJson(result)) > maxBytes) throw new BehaviorContractError('behavior.byte-budget');
  return result;
}
